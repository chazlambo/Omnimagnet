...
for i in range(1,iters):
    ...
    for j in range(0,4):
        arm_avg[0][j] = PC1.read(j)
        arm_avg[1][j] = PC2.read(j)
        arm_avg[2][j] = PC3.read(j)
        arm_avg[3][j] = PC4.read(j)
        arm_avg[4][j] = PC5.read(j)
    ...
    print(i)
    print("Arm # \t Horz \t Wrist \t Base \t Vertical")
    for a in range(0,5):
        for b in range(0,4):
            arm_avg[a][b] = arm_avg[a][b];
